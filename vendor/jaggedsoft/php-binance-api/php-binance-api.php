...
class API
{
    ...
    {
       $ticker = $this->httpRequest("https://api.binance.com/api/v3/ticker/price", "GET");
       return $ticker;
    ...
    protected function httpRequest(string $url, string $method = "GET", array $params = [], bool $signed = false)
    {
        ...
        if (curl_errno($curl) > 0) {
            ...
            Toastr::warning('Market Not Responding.', 'Ohh Sorry!');
            return back();
        }
        ...
        if (isset($json['msg']) && !empty($json['msg'])) {
            if ( $url != 'v1/system/status' && $url != 'v3/systemStatus.html' && $url != 'v3/accountStatus.html') {
                ...
                Toastr::warning('Market Not Responding, Please Try again later.', 'Ohh Sorry!');
                return back();
            }
        }
